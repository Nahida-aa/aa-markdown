...
1. 阅读本阶段的行为要求。
2. 先用 TypeScript/JavaScript 的经验写下自己的设计猜想。
3. 亲自创建文件并实现代码。
4. 运行格式化、测试和 Clippy。
5. 遇到编译错误时，先阅读错误信息，再向教练提问。
...
注意：这些文件尚未被 Git 跟踪时，`git diff -- Cargo.toml` 不会显示它们的内容；这是 Git 对未跟踪文件的正常行为。当前阶段不要求提交 Git commit。把 `git status`、忽略检查和所有验证命令的结果发给我，下一阶段再一起定义第一个公开 API，而不是提前猜测完整的 Markdown 设计。